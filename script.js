/**
 * ============================================================
 *  📱 Instagram Profile Cleaner  v4.0
 * ============================================================
...
(async function InstaProfileCleaner() {
  ...
  const getIgClaim = () =>
    localStorage.getItem('ig-www-claim') ||
    localStorage.getItem('claim') ||
    getCookie('ig-www-claim') || '0';

  const HEADERS = {
    'x-csrftoken': getCookie('csrftoken'),
    'x-ig-app-id': '936619743392459',
    'x-ig-www-claim': getIgClaim(),
    'x-asbd-id': '129477',
  ...
  const loadFollowers = uid =>
    fetchAllPages(
      mid => apiGet(`/api/v1/friendships/${uid}/followers/?count=200${mid ? `&max_id=${mid}` : ''}`),
      n => setStatus(`📥 Follower: ${n}...`)
    );

  const loadFollowing = uid =>
    fetchAllPages(
      mid => apiGet(`/api/v1/friendships/${uid}/following/?count=200${mid ? `&max_id=${mid}` : ''}`),
      n => setStatus(`📥 Following: ${n}...`)
    );

  const getUserInfo = async uid => {
    const d = await withRetry(() => apiGet(`/api/v1/users/${uid}/info/`));
    return d.user;
  };

  const getLastPostDate = async uid => {
    try {
      const d = await withRetry(() => apiGet(`/api/v1/feed/user/${uid}/?count=1`));
      const items = d.items || [];
  ...
  const getMyPosts = async (uid, count = 12) => {
    const d = await withRetry(() => apiGet(`/api/v1/feed/user/${uid}/?count=${count}`));
    return d.items || [];
  };

  const getPostLikers = async mediaId => {
    try {
      const d = await withRetry(() => apiGet(`/api/v1/media/${mediaId}/likers/`));
      return d.users || [];
    } catch { return []; }
  };
  ...
  const getPostCommenters = async mediaId => {
    try {
      const d = await withRetry(() => apiGet(`/api/v1/media/${mediaId}/comments/?count=100`));
      return (d.comments || []).map(c => c.user);
    } catch { return []; }
  };

  const removeFollower = uid =>
    apiPost(`/api/v1/friendships/remove_follower/${uid}/`, `user_id=${uid}`);

  const unfollow = uid =>
    apiPost(`/api/v1/friendships/destroy/${uid}/`, `user_id=${uid}`);
  ...
  function exportCSV() {
    ...
    for (const u of state.results) {
      rows.push([
        u.username, u.full_name || '', u.follower_count ?? '', u.following_count ?? '',
        fanScore(u), u.reason,
        u.last_post_date ? formatDate(u.last_post_date) : '', u.is_private ? 'Sì' : 'No',
      ]);
    }
  ...
  async function scanFollowers() {
    ...
    }, (cur, tot) => setProgress(cur, tot, `Analisi follower: ${cur}/${tot}`));
  ...
  async function scanFollowing() {
    ...
    for (const u of state.following) {
      ...
      if (state.settings.g_nonFollowers && followerSet && !followerSet.has(String(u.pk))) {
        addResult(u, '🔄 Non ti segue', 'nonfollower'); tagged = true;
      }
      if (state.settings.g_private && u.is_private && !tagged) {
        addResult(u, '🔒 Account privato', 'private'); tagged = true;
      }
  ...
  async function scanInteractions() {
    ...
    if (!posts.length) { setStatus('❌ Nessun post trovato. Hai post pubblici?'); return; }
  ...
  const CSS = `
    #ipc-root *, #ipc-root *::before, #ipc-root *::after { box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; }
    #ipc-root { position: fixed; top: 14px; right: 14px; z-index: 999999; width: 430px; max-height: 93vh; background: #fff; border-radius: 18px; box-shadow: 0 14px 52px rgba(0,0,0,0.24); display: flex; flex-direction: column; overflow: hidden; border: 1px solid #e0e0e0; }
    #ipc-header { background: linear-gradient(135deg, #833ab4 0%, #fd1d1d 55%, #fcb045 100%); color: #fff; padding: 13px 16px; display: flex; align-items: center; justify-content: space-between; flex-shrink: 0; }
    ...
    .ipc-icon-btn { background: rgba(255,255,255,0.2); border: none; border-radius: 8px; color: #fff; cursor: pointer; padding: 4px 8px; font-size: 13px; }
    ...
    .ipc-tab { flex: 1; padding: 9px 4px; border: none; background: none; font-size: 12px; font-weight: 600; color: #999; cursor: pointer; border-bottom: 2px solid transparent; transition: all 0.15s; }
    .ipc-tab.active { color: #833ab4; border-bottom-color: #833ab4; background: #fff; }
    #ipc-filters { padding: 10px 14px 8px; border-bottom: 1px solid #f0f0f0; background: #fafafa; flex-shrink: 0; }
    .ipc-chips { display: flex; flex-wrap: wrap; gap: 5px; }
    .ipc-chip { background: #fff; border: 1.5px solid #ddd; border-radius: 20px; padding: 3px 10px; font-size: 11px; font-weight: 600; cursor: pointer; color: #555; transition: all 0.15s; user-select: none; }
    ...
    .ipc-sub select, .ipc-sub input[type=number] { border: 1px solid #ddd; border-radius: 7px; padding: 2px 7px; font-size: 11px; background: #fff; color: #333; }
    .ipc-sub input[type=number] { width: 54px; }
    #ipc-scan { margin: 8px 14px; padding: 9px; background: linear-gradient(135deg, #833ab4, #fd1d1d); color: #fff; border: none; border-radius: 10px; font-size: 13px; font-weight: 700; cursor: pointer; flex-shrink: 0; transition: opacity 0.2s; }
    ...
    #ipc-stats { display: flex; flex-wrap: wrap; gap: 5px; padding: 4px 14px 6px; flex-shrink: 0; border-bottom: 1px solid #f5f5f5; }
    .ipc-stat { background: #f5f5f5; border-radius: 20px; padding: 2px 9px; font-size: 10px; color: #555; }
    .ipc-stat b { color: #333; }
    #ipc-toolbar { display: none; flex-wrap: wrap; gap: 5px; padding: 6px 14px; flex-shrink: 0; }
    #ipc-search { flex: 1; min-width: 100px; padding: 6px 10px; border: 1.5px solid #ddd; border-radius: 8px; font-size: 12px; color: #333; }
    #ipc-search:focus { outline: none; border-color: #833ab4; }
    #ipc-sort { padding: 6px 8px; border: 1.5px solid #ddd; border-radius: 8px; font-size: 12px; background: #fff; color: #333; cursor: pointer; }
    #ipc-export-btn { padding: 6px 10px; border: 1.5px solid #ddd; border-radius: 8px; background: #fff; font-size: 12px; cursor: pointer; color: #555; white-space: nowrap; }
    #ipc-export-btn:hover { background: #f5f5f5; }
    #ipc-toolbar-row2 { display: flex; gap: 5px; width: 100%; }
    #ipc-select-all { flex: 1; padding: 6px; font-size: 12px; font-weight: 600; border: 1.5px solid #ddd; border-radius: 8px; background: #fff; cursor: pointer; color: #333; }
    #ipc-select-all:hover { background: #f5f5f5; }
    #ipc-action-btn { flex: 2; padding: 6px 8px; font-size: 12px; font-weight: 700; background: #ed4956; color: #fff; border: none; border-radius: 8px; cursor: pointer; }
    ...
    #ipc-safe-badge { font-size: 10px; padding: 2px 9px; background: #e8f5e9; color: #2e7d32; border-radius: 20px; flex-shrink: 0; align-self: center; }
    ...
    .ipc-user { display: flex; align-items: center; gap: 8px; padding: 7px 8px; border-radius: 10px; cursor: pointer; transition: background 0.1s; }
    .ipc-user:hover { background: #f8f8f8; }
    .ipc-user.sel { background: #fff0f0; }
    .ipc-user img { width: 38px; height: 38px; border-radius: 50%; object-fit: cover; flex-shrink: 0; border: 1.5px solid #eee; }
    ...
    #ipc-settings-panel { display: none; position: absolute; top: 0; left: 0; right: 0; bottom: 0; background: #fff; z-index: 10; border-radius: 18px; overflow-y: auto; padding: 16px; }
    #ipc-settings-panel h3 { font-size: 14px; margin-bottom: 14px; }
    .ipc-set-row { display: flex; align-items: center; justify-content: space-between; padding: 8px 0; border-bottom: 1px solid #f5f5f5; font-size: 13px; color: #333; }
    .ipc-set-row:last-child { border-bottom: none; }
    .ipc-set-row input[type=checkbox] { width: 18px; height: 18px; cursor: pointer; }
    .ipc-set-row input[type=number], .ipc-set-row select { border: 1px solid #ddd; border-radius: 8px; padding: 3px 8px; font-size: 12px; width: 80px; }
    .ipc-set-desc { font-size: 11px; color: #aaa; margin-top: 1px; }
    #ipc-settings-close { width: 100%; padding: 10px; margin-top: 14px; background: linear-gradient(135deg, #833ab4, #fd1d1d); color: #fff; border: none; border-radius: 10px; font-size: 13px; font-weight: 700; cursor: pointer; }
  `;
  ...
  function buildUI() {
    ...
    root.innerHTML = `
      ...
      <div id="ipc-settings-panel">
        ...
        <div class="ipc-set-row"><div><div>Safe Mode</div><div class="ipc-set-desc">Limita rimozioni giornaliere</div></div><input type="checkbox" id="cfg-safeMode" ${state.settings.safeMode?'checked':''}></div>
        <div class="ipc-set-row"><div><div>Limite giornaliero</div><div class="ipc-set-desc">Max rimozioni al giorno</div></div><input type="number" id="cfg-dailyLimit" min="5" max="200" value="${state.settings.dailyLimit}"></div>
        <div class="ipc-set-row"><div><div>Batch size</div><div class="ipc-set-desc">Richieste parallele (2–5)</div></div><input type="number" id="cfg-batchSize" min="2" max="5" value="${state.settings.batchSize}"></div>
        <div class="ipc-set-row"><div><div>Pausa tra batch (ms)</div><div class="ipc-set-desc">Più basso = più veloce ma più rischioso</div></div><input type="number" id="cfg-batchDelay" min="800" max="5000" step="100" value="${state.settings.batchDelay}"></div>
        <div class="ipc-set-row"><div><div>Auto-whitelist verificati</div><div class="ipc-set-desc">Esclude account con ✓ blu</div></div><input type="checkbox" id="cfg-autoVerified" ${state.settings.autoWhitelistVerified?'checked':''}></div>
        <div class="ipc-set-row"><div><div>Post da analizzare</div><div class="ipc-set-desc">Per scansione Interazioni</div></div><select id="cfg-postsToCheck"><option value="3" ${state.settings.i_postsToCheck===3?'selected':''}>3 post</option><option value="6" ${state.settings.i_postsToCheck===6?'selected':''}>6 post</option><option value="12" ${state.settings.i_postsToCheck===12?'selected':''}>12 post</option><option value="24" ${state.settings.i_postsToCheck===24?'selected':''}>24 post</option></select></div>
  ...
  function bindEvents() {
    ...
    root.querySelectorAll('.ipc-tab').forEach(btn => {
      btn.onclick = () => {
        ...
        state.selected.clear(); state.searchQuery = '';
        buildUI();
      };
    });
    ...
    root.querySelector('#ipc-scan').onclick = async () => {
      ...
      state.scanning = true;
      state.results = []; state.filteredResults = []; state.selected.clear(); state.searchQuery = '';
    ...
    if (search) search.oninput = e => { state.searchQuery = e.target.value; applyFilter(); };
    const sort = root.querySelector('#ipc-sort');
    if (sort) { sort.value = state.sortKey; sort.onchange = e => { state.sortKey = e.target.value; applyFilter(); }; }
    ...
    root.querySelector('#ipc-select-all').onclick = () => {
      if (state.selected.size === state.filteredResults.length) state.selected.clear();
      else state.filteredResults.forEach(u => state.selected.add(u.pk));
      renderResults();
    };
    root.querySelector('#ipc-action-btn').onclick = executeAction;
    root.querySelector('#ipc-settings-btn').onclick = () => { root.querySelector('#ipc-settings-panel').style.display = 'block'; };
  ...
  function setStatus(msg) { const el = root.querySelector('#ipc-status'); if (el) el.textContent = msg; }
  ...
  const formatDate = d => d?.toLocaleDateString('it-IT', { day: '2-digit', month: 'short', year: 'numeric' }) || '';
  ...
  function renderResults() {
    ...
    if (countEl) {
      const shown = users.length, total = state.results.length;
      countEl.textContent = shown === total ? `${total} account trovati` : `${shown} di ${total} (filtrati)`;
    }
    if (actionBtn) { actionBtn.disabled = selCount === 0; actionBtn.textContent = `${actionLabel} (${selCount})`; }
    if (selAllBtn) selAllBtn.textContent = selCount === users.length ? '☑ Tutti' : '☐ Tutti';
      ...
      return `
        <div class="ipc-user ${sel?'sel':''}" data-id="${u.pk}">
          ...
          <div style="display:flex;flex-direction:column;align-items:flex-end;gap:3px;flex-shrink:0;">
            ...
            ${u.following_count > 0 ? `<span class="ipc-fs ${fsClass}" title="Fan score">fs ${fs}</span>` : ''}
          </div>
